...
app.get('/api/public/baroApi', async (req, res) => {
  ...
  try {
    ...
    if (trimmedText.startsWith('<?xml') || trimmedText.startsWith('<response') || trimmedText.startsWith('<result')) {
      ...
      const errorMatch = trimmedText.match(/<message>([^<]+)<\/message>/i) || 
                        trimmedText.match(/<resultMsg>([^<]+)<\/resultMsg>/i);
    ...
    if (responseData.body?.items) {
      ...
      if (itemsData.item) {
        items = Array.isArray(itemsData.item) 
          ? itemsData.item 
          : [itemsData.item].filter(Boolean);
      }
    } else if (responseData.items) {
      items = Array.isArray(responseData.items.item) 
        ? responseData.items.item 
        : [responseData.items.item].filter(Boolean);
  ...
  try {
    ...
    try {
      ...
      if (!baroResponseText.trim().startsWith('<!DOCTYPE') && !baroResponseText.trim().startsWith('<html')) {
        ...
        if (baroData.body?.items?.item) {
          baroItems = Array.isArray(baroData.body.items.item) 
            ? baroData.body.items.item 
            : [baroData.body.items.item].filter(Boolean);
        } else if (baroData.items?.item) {
          baroItems = Array.isArray(baroData.items.item) 
            ? baroData.items.item 
            : [baroData.items.item].filter(Boolean);
        }
    ...
    try {
      ...
      if (trimmedText.startsWith('<?xml') || trimmedText.startsWith('<response') || trimmedText.startsWith('<result')) {
        ...
        const errorMatch = trimmedText.match(/<message>([^<]+)<\/message>/i) || 
                          trimmedText.match(/<resultMsg>([^<]+)<\/resultMsg>/i) ||
                          trimmedText.match(/<returnAuthMsg>([^<]+)<\/returnAuthMsg>/i);
    ...
    if (error.response?.data && typeof error.response.data === 'string') {
      ...
      if (responseText.startsWith('<!DOCTYPE') || responseText.startsWith('<html')) {
        ...
        errorDetails = {
          ...
          commonIssues: [
            'API 엔드포인트 경로가 올바른지 확인 (서비스 메서드명 포함 여부)',
            '필수 파라미터가 모두 포함되었는지 확인',
            '인증키(serviceKey)가 올바른지 확인',
            'API 서비스가 활성화되어 있는지 확인',
            'resultType=json 파라미터가 포함되었는지 확인',
          ],
      ...
      // XML 응답 처리
        ...
        const errorMatch = responseText.match(/<message>([^<]+)<\/message>/i) || 
                          responseText.match(/<resultMsg>([^<]+)<\/resultMsg>/i) ||
                          responseText.match(/<returnAuthMsg>([^<]+)<\/returnAuthMsg>/i) ||
                          responseText.match(/<msg>([^<]+)<\/msg>/i) ||
                          responseText.match(/<errorMsg>([^<]+)<\/errorMsg>/i);
